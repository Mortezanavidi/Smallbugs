...
class Project_Issue_Controller extends Base_Controller {
	...
	public function post_new() {
		...
		//Email process
			...
			$text .= "\n\n";
			$text .= sprintf(__('smallbugs.priority')." : ".__('smallbugs.priority_desc_'.$thisIssue[0]->attributes["status"]));
			$text .= "\n\n";
		...
		return Redirect::to($issue['issue']->to())
			->with('notice', __('smallbugs.issue_has_been_created'));
	}
	...
	public function post_index() {
		if(!Input::get('comment')) {
			return Redirect::to(Project\Issue::current()->to() . '#new-comment')
				->with('notice-error', __('smallbugs.you_put_no_comment'));
		}
		...
		return Redirect::to(Project\Issue::current()->to() . '#comment' . $comment->id)
			->with('notice', __('smallbugs.your_comment_added'));
	}
	...
	public function post_edit() {
		...
		return Redirect::to(Project\Issue::current()->to())
			->with('notice', __('smallbugs.issue_has_been_updated'));
	}
	...
	public function get_delete_comment() {
		...
		return Redirect::to(Project\Issue::current()->to())
			->with('notice', __('smallbugs.comment_deleted'));
	}
	...
	public function get_status() {
		...
		if($status == 0) {
			$message = __('smallbugs.issue_has_been_closed');
		} else {
			$message = __('smallbugs.issue_has_been_reopened');
		}
	...
	public function get_reassign() {
		if (Input::get('Suiv') == 0 ) { $result = false; } else {
			...
			$result  = __('smallbugs.edit_issue')." : ";
			...
			$t = time();
				...
				$content .= __('smallbugs.label_reassigned').'</label>&nbsp;';
				$content .= __('smallbugs.to') . ' ';
				$content .= ' <b>'.$WhoName.'</b> ';
				$content .= __('smallbugs.by') . ' ';
	...
	public function get_retag() {
			...
			if ($Modif == 'eraseTag') {	
				...
				$Action = $Issue;
				$Modif = true;
				$Msg = '<span style="color:#F00;">'.__('smallbugs.tag_removed').'</span>';
				$Show = true;
			}
	...
	public function post_upload() {
		...
		if (is_numeric($msg)) {
			...
			$msg .= '<span style="font-weight: bold; color: #090;">'.__('smallbugs.fileuploaded').'</span>';
