			...
			<form method="get" action="">
				<div class="filter-and-sorting_TAGS">
					<div style="position: absolute; left: 0; width: 10%; font-weight: bold; text-align: right; top: 13px;">
						<b><?php echo __('smallbugs.tags'); ?></b>
					</div>
				...
				<div class="filter-and-sorting_BAS">
					...
					<div class="filter-and-sorting_FILTER" >
						<div style="position: relative; left: -15%; width: 15%; font-weight: bold; text-align: left; top: 3px; display: inline; margin-right: -10%;">
							<b><?php echo __('smallbugs.limits'); ?></b>
						</div>
						<?php echo Form::select('limit_contrib', array( 'assigned_to' => __('smallbugs.limits_contrib_assignedto'),'created_by'  => __('smallbugs.limits_contrib_createdBy'),'closed_by'   => __('smallbugs.limits_contrib_closedby'),'updated_by'  => __('smallbugs.limits_contrib_updatedby')), Input::get('limit_contrib', '')); ?>
						<?php echo Form::select('assigned_to', $assigned_users, Input::get('assigned_to', '')); ?>
						<br /><br />
						<?php echo Form::select('limit_event', array('created_at' => __('smallbugs.limits_event_createdat'),'updated_at' => __('smallbugs.limits_event_updatedAt'),'closed_at'  => __('smallbugs.limits_event_closedAt')), Input::get('limit_event', '')); ?>
						<?php echo Form::select('limit_period',array('' => "", 'week' 	=> __('smallbugs.limits_period_week'),'month' 	=> __('smallbugs.limits_period_month'),'months' => __('smallbugs.limits_period_months')), Input::get('limit_period', ''),array("onchange"=>"CalculonsDates(this.value);" ) ); ?>
						<br /><br />
					...
					<div class="filter-and-sorting_SORT">
						<div style="position: absolute; left: -25%; width: 25%; font-weight: bold; text-align: left; top: 2px; display: inline-block;">
							<b><?php echo __('smallbugs.sort_by'); ?></b>
						</div>
						...
						<?php echo Form::select('sort_order', array('asc' => __('smallbugs.sort_asc'), 'desc' => __('smallbugs.sort_desc')), $sort_order); ?>
						<input name="tag_id" value="<?php echo Input::get('tag_id', '1'); ?>" type="hidden" />
						<br />
						<br /><br />
						<input type="submit" value="<?php echo __('smallbugs.show_results'); ?>" class="button primary" />
					</div>
...
<div class="blue-box">
	<div class="inside-pad">
		<?php if(!$issues): ?>
		<p><?php echo __('smallbugs.no_issues'); ?></p>
		<?php else: ?>
		<ul class="issues" id="sortable">
			...
			<li class="sortable-li" data-issue-id="<?php echo $row->id; ?>">
				...
				<div class="data">
					...
					<div class="info">
						<?php echo __('smallbugs.created_by'); ?>
						...
						<?php if(!is_null($row->updated_by)): ?>
							- <?php echo __('smallbugs.updated_by'); ?>
						...
						<?php if($row->assigned_to != 0): ?>
							- <?php echo __('smallbugs.assigned_to'); ?>
					...
					if (@$_GET["tag_id"] == 1) {
						echo '<br /><br />';
								...
								$SizeXtot = 500;
								$SizeX = $SizeXtot / 100;
//								echo __('smallbugs.issue_percent').' : ';
...
//								echo __('smallbugs.countdown').' ('.__('smallbugs.day').'s) : ';
								echo '<div class="Percent2">';
		...
		<?php endif; ?>
		<div id="sortable-msg"><?php echo __('smallbugs.sortable_issue_howto'); ?></div>
		<div id="sortable-save"><input id="sortable-save-button" class="button primary" type="submit" value="<?php echo __('smallbugs.save'); ?>" /></div>
	</div>
</div>
<script type="text/javascript">
