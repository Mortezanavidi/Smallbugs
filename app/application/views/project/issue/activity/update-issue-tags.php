		...
		<div class="topbar">
			<div class="data">
				<?php
					...
					$Msg = ($activity->attributes['parent_id'] == $activity->attributes['item_id'] ) ? __('smallbugs.tag_removed') : __('smallbugs.tag_added');
					...
					'</span> '.__('smallbugs.by') . ' <b>' .$Who[0]->attributes["firstname"].' '.$Who[0]->attributes["lastname"].'</b> '.
					' '.date(Config::get('application.my_bugs_app.date_format'), strtotime($activity->attributes['updated_at']));
						...
						echo count($j['added_tags']).' '.((count($j['added_tags']) > 1) ? __('smallbugs.tags_added') : __('smallbugs.tag_added')).'.';
						...
						echo '<br clear="all" />';
						echo '<br clear="all" />';
						echo count($j['removed_tags']).' '.((count($j['removed_tags']) > 1) ? __('smallbugs.tags_removed') : __('smallbugs.tag_removed')).'.';
						foreach ($j['tag_data'] as $ind => $val ) { 
