...
<h3>
	...
	<span><?php echo __('smallbugs.on_project'); ?> <a href="<?php echo $project->to(); ?>"><?php echo $project->name; ?></a></span>
</h3>
<div class="pad">
		...
		<li>
			<div class="insides">
				<div class="topbar">
					...
					<?php echo __('smallbugs.opened_this_issue'); ?>  <?php echo date(Config::get('application.my_bugs_app.date_format'), strtotime($issue->created_at)); ?>
				</div>
	...
	<div class="new-comment" id="new-comment">
		<?php if(Auth::user()->permission('issue-modify')): ?>
			...
			<ul class="issue-actions">
				<li class="assigned-to">
					...
					<div class="dropdown">
						<ul id="dropdown_ul">
							<li class="unassigned" id="dropdown_li_0"><a href="javascript: Reassignment(<?php echo $project->id.','.((Project\Issue::current()->assigned_to == '') ? 0 : Project\Issue::current()->assigned_id).',0,'.Project\Issue::current()->id; ?>);" class="user0<?php echo !Project\Issue::current()->assigned_id ? ' assigned' : ''; ?>" ><?php echo __('smallbugs.no_one'); ?></a></li>
							<?php 
				...
				<li>
					...
					<a href="<?php echo Project\Issue::current()->to('status?status=0'); ?>" onclick="return confirm('<?php echo __('smallbugs.close_issue_confirm'); ?>');" class="close"><?php echo __('smallbugs.close_issue'); ?></a>
					<?php } else { echo '&nbsp;'; } ?>
				</li>
			</ul>
		<?php endif; ?>
		...
		<h4>
			<?php echo __('smallbugs.comment_on_this_issue'); ?>
		</h4>
		...
		<form method="post" action="" enctype="multipart/form-data">
			<p>
				...
				<?php 
					...
					if (Project\Issue::current()->assigned->id == \Auth::user()->id ) { 
						echo __('smallbugs.percentage_of_work_done').':';
			...
			<ul id="uploaded-attachments">
				<p>
					<div id="div_upload" class="upload-wrap green-button" onclick="document.getElementById('div_upload').style.width = '280px'; document.getElementById('span_butupload').style.display = 'block'; ">
						<span id="upload_title"><?php echo __('smallbugs.fileupload_button'); ?></span>
						<span id="span_butupload" style="display: none;">
							...
							<input type="hidden" id="uploadbuttontext" name="uploadbuttontext" value="<?php echo __('smallbugs.fileupload_button'); ?>" style="color:#000; background-color:#99F;" />
						</span>
					</div>
			...
			<p style="margin-top: 10px;">
				<input type="submit" class="button primary" value="<?php echo __('smallbugs.comment'); ?>" />
			</p>
	...
	<?php else: ?>
	<?php echo HTML::link(Project\Issue::current()->to('status?status=1'), __('smallbugs.reopen_issue')); ?>
	<br /><br />
	<?php endif; ?>
	<br /><br />
</div>
	   ...
	   if (this.readyState == 4 && this.status == 200) {
			...
			if (this.readyState == 4 && this.status == 200) {
				...
				if ( bons.indexOf(resultat) > 0 ) {
						...
						msg = '<?php echo __('smallbugs.fileupload_failed'); ?><br />' + recu.substr(3);
